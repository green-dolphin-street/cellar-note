...
export default function App(){
  ...
  const [syncing,setSyncing]=useState(false),[serverReady,setServerReady]=useState(false),[hydrated,setHydrated]=useState(false);
  ...
  async function syncThisDevice(){
    ...
    try{
      ...
      await loadSharedWines();setNotice(`${local.length}개의 이 기기 기록을 공용 보관함과 동기화했습니다.`);
    }catch(error){setNotice(error.message)}finally{setSyncing(false)}
  }
  async function login(e){e.preventDefault();const r=await fetch("/api/auth",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({code})});if(r.ok){sessionStorage.setItem("cellar-note-upload-code",code);setAccount(false);setCode("");setNotice("사진 분석 로그인이 완료되었습니다.")}else setNotice("로그인 정보가 올바르지 않습니다.")}
  ...
  return <main>
    ...
    {account&&<div className="overlay"><form className="account" onSubmit={login}><button type="button" className="x" onClick={()=>setAccount(false)}>×</button><h2>MY</h2><p>공개 열람은 누구나 가능하고 사진 분석과 기록 변경만 로그인이 필요합니다.</p>{unlocked()?<><b>{serverReady?"공용 보관함 연결됨":"이 기기의 임시 기록을 표시 중"}</b><small>이번 달 이 브라우저의 예상 사용액: ${(JSON.parse(localStorage.getItem("cellar-note-usage")||"{}")[new Date().toISOString().slice(0,7)]||0).toFixed(3)}</small><button type="button" disabled={syncing} onClick={syncThisDevice}>{syncing?"동기화 중…":"이 기기의 기존 기록 동기화"}</button><small>예전에 이 기기에서 저장한 와인과 사진을 공용 보관함으로 옮깁니다.</small><button type="button" className="quiet" onClick={()=>{sessionStorage.removeItem("cellar-note-upload-code");setAccount(false)}}>로그아웃</button></>:<><label>관리자 로그인<input type="password" value={code} onChange={e=>setCode(e.target.value)} required/></label><button>로그인</button></>}</form></div>}
