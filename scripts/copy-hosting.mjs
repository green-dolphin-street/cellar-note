...
async function saveWine(request,env){
 ...
 const id=String(wine.id).replace(/[^a-zA-Z0-9_-]/g,"-"),data={...wine,id},incoming=Array.isArray(data.images)&&data.images.length?data.images:[data.image].filter(Boolean),stored=[];
 for(let index=0;index<incoming.length;index++){let value=incoming[index];if(value.startsWith("data:image/")){const filename=id+"-"+index+".jpg",path="/storage/v1/object/wine-images/"+encodeURIComponent(filename),upload=await dbFetch(env,path,{method:"POST",headers:{"content-type":"image/jpeg","x-upsert":"true"},body:imageBytes(value)});if(!upload.ok)return json({error:"사진 저장에 실패했습니다: "+(await upload.text()).slice(0,180)},502);value=env.SUPABASE_URL.replace(/\\/$/,"")+"/storage/v1/object/public/wine-images/"+encodeURIComponent(filename)}stored.push(value)}
 const imageUrl=stored[0]||data.image||"";data.images=stored;delete data.image;
